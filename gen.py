...
def circle_point(p, radius, lateral_angle, vertical_angle):
    _pp = np.dot(rotation_matrix(p, lateral_angle), perpendicular_vector(p))
    _pp = np.dot(rotation_matrix(np.cross(p, _pp), vertical_angle), _pp)
...
def line_to_curvy_path(
    ...
    return bspline(vary_path(
        split_line(p0, p1, division),
        f_radius=f_variation,
        f_angle=f_angle
    ), n=n)

def curvy_path_from_point(
    point,
    direction,
    length,
    lateral_angle,
    ...
    return line_to_curvy_path(
        point, point+circle_point(direction, length, lateral_angle, vertical_angle),
        division=variation_division,
...
def gen_tree(p0, p1, max_depth=2, **kwargs):
    def _gen_tree(_tree={}, **kwargs):
        ...
        _tree['children'] = [
            ...
            for l_acc, (_p0, _p1) in with_accumulated_length_fractions(_tree.get('path'))
        ...
        **kwargs,
        _tree = {
            'path': line_to_curvy_path(
                p0, p1,
                division = kwargs.get('f_variation_division')(0),
